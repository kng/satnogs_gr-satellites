...
# Description: Generic FM demodulation flowgraph
...
class satnogs_fm(gr.top_block):

    def __init__(self, antenna="", bb_freq=0.0, bw=0.0, dc_removal="False", decoded_data_file_path="/tmp/.satnogs/data/data", dev_args="", deviation=5e3, doppler_correction_per_sec=20, enable_iq_dump=0, file_path="test.wav", gain=0.0, gain_mode="Overall", iq_file_path="/tmp/iq.dat", lo_offset=100e3, max_modulation_freq=3e3, other_settings="", ppm=0, rigctl_port=4532, rx_freq=100e6, samp_rate_rx=0.0, soapy_rx_device="driver=invalid", stream_args="", tune_args="", udp_IP="127.0.0.1", udp_dump_dest='127.0.0.1', udp_dump_port=7355, udp_port=16887, waterfall_file_path="/tmp/waterfall.dat"):
        ...
        self.decimation = decimation = max(4,satnogs.find_decimation(audio_samp_rate, 2, audio_samp_rate))
    ...
    def set_audio_samp_rate(self, audio_samp_rate):
        ...
        self.set_decimation(max(4,satnogs.find_decimation(self.audio_samp_rate, 2, self.audio_samp_rate)))
        self.analog_quadrature_demod_cf_0.set_gain((2*math.pi*self.deviation)/self.audio_samp_rate)
        self.low_pass_filter_0.set_taps(firdes.low_pass(1, self.audio_samp_rate, self.deviation+self.max_modulation_freq, 1000, firdes.WIN_HAMMING, 6.76))
...
def argument_parser():
    ...
    parser.add_argument(
        ...
        help="Set The highest frequency in the modulating signal [default=%(default)r]")
    parser.add_argument(
...
def main(top_block_cls=satnogs_fm, options=None):
    if options is None:
